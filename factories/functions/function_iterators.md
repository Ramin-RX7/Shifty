...
> The reason behind having function iterators beside normal iterators (build with `META:iter`) is that some iterators might be type independent.
...
> Before you see how this exactly works it's better to check how [[for_loop]] actually work.
...
This means the function will start executing until it reaches a `yield` statement. Now it's time to save all data of the function for now, pause the execution (for later resume), and return the value in front of `yield`.
