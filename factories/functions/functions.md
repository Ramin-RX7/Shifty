...
Although parameter types can be defined using a single simple type, functions can have different conditions and policies for each of their parameters using [[types#Unions|Unions]] and other [[type_validation#Conditionals|Conditionals]].

When declaring the function prototype, if the argument type is not a type which inherits from `Conditional`, By default it will be translated to `Castable(GIVEN_TYPE)` and implicit casting will be applied in calls. Read [[type_validation|more]]
...
`Task` is a class that will be initialized whenever a factory with type of `function` is created. It will be saved in the factory `data` mapping. In fact, whenever the function is called, a method of the initialized object we just mentioned will be called.
...
Note that in the `Constructor` of `Task`, parameter `parameters` is a tuple with this structure:
...
- Validate given arguments:
  + validate if required arguments are all given
