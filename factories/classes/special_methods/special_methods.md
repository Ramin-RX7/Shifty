...
- **hash**: to get a hash of the instance (this method will be used when `OP:equals` is not defined)
- **includes**: used in "`in`" expression.
- **iter**: will be called in each iteration of for loop
- **call**: when the instance is called
...
> (waiting for decision) Should there be a method to initialize the instance implicitly when a variable is declared but not initialized by the user? if the language allow user to declare a variable without explicitly initializing it, this feature must exist.

> read more about [[metas|Meta Methods]]
...
> read more about [[operators|Operator methods]]
...
read complete doc of [[casting|Casting methods]]
