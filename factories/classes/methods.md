...
> They are documented in [[special_methods|Special Methods]]

# Acquiring the methods

Methods can be decorated with a *[[acquiring|Acquire]]* utility to be given permission on using data container of class, object and even the themselves.
...
class MyClass {
    #acquire(self)
    func my_method(){
        // here we have access to the instance
    }
}
```
...
> [!attention]
> When using `#acquire` explicitly, it will replace the default.

Not only the `self` which is the instance can be acquired, but also the class and the method itself can also be acquired.
