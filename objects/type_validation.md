...
In [[types]] we talked about `Unions` (and `Type Aliases`).
...
> This is the base class for type validation checks that all sub-classes must implement an abstract method, defined for checking the conditions.
> Below types (also Union) must all inherit from this class.
