...
> [!attention] The syntax used above for Unions is just for example purposes (since syntax of the language is not implemented yet)
...
Let's say we have a function called `f` with parameter `x` where `x` can be an `int` or a `float`. This could easily be done using Unions:
```
func f(x:int|float){...}
```
...
# Difference of Unions and Type-Aliases

Basically type aliases work the same as Unions but the difference is that the Union is an object type itself, while Type-Alias is what we call a variable that points to an Union object.

Union is referred to the object that exist in the OM while Type Alias is called to a variable in VM which uses a reference of an object of type Union in OM.