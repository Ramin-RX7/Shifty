...
This document covers all primitive types of `Shifty`
...
`Boolean` must be a built-in enum in the language, which only contains 2 elements: `true` and `false`.
These two determine whether a condition is met or not.
