...
# ValidationError
_derived from [[#BaseError]]_

Thrown when a validation fails. It is used when a data input is unknown and when it is received, validations on it fails. This class is mostly supposed to be derived by other classes (like `InvalidValueError` and `InvalidTypeError`) but still enough for some errors.
...
> (Waiting for decision) This error might be better to be beside BaseError instead of being derived from it.
# InternalError
_derived from [BaseError](#baseerror)_
